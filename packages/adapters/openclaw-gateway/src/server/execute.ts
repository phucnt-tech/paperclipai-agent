...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    ...envLines,
    "PAPERCLIP_API_KEY=<already provided in adapter env vars, if present>",
    "",
    "Auth key rule (strict):",
    "- If PAPERCLIP_API_KEY exists in environment, use it directly.",
    `- Only if missing from env, load it from ${claimedApiKeyPath}.`,
    "- Do not invent or alter key-file paths.",
    "",
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const disableDeviceAuth = parseBoolean(parsedConfig.disableDeviceAuth, false);
